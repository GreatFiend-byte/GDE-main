...
const ResuelveExamen = () => {
  ...
  useEffect(() => {
    const cargarPreguntasYRespuestas = async () => {
      ...
      try {
        ...
        const preguntasResponse = await fetch(`http://localhost:8000/descargar-examen/${selectedExamen}`);
        ...
        try {
          const respuestasResponse = await fetch(`http://localhost:8000/obtener-respuestas/${selectedExamen}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/guardar-respuestas', {
        method: 'POST',
        headers: {
